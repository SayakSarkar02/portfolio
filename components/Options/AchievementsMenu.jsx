...
const MenuCard = ({keyMap, desc, logo, links, stacks, title, data}) => {
  ...
  return(
    <div key={keyMap}
    onClick={()=>{dispatch(setBody({data, type:"achievements"}));
...
const Achievements = () => {
  ...
  const projectsList = useSelector((state) => state.menu.achievements);
  console.log(projectsList);

  return (
    <>
      <div className="flex flex-row items-center justify-between mx-5">
        <h3 className=" font-sans text-vs-white-1 font-light">ACHIEVEMENTS</h3>
